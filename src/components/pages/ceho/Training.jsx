...
function ManageTrainings() {
  ...
  const filteredData = trainingData.filter(
    (training) =>
      training.created_by.phone?.toLowerCase().includes(searchQuery.toLowerCase()) ||
              ...
              currentTrainings.map((training) => (
                <tr key={training.id} className="bg-white border-b">
                  ...
                  <td className="px-6 py-4 flex items-center space-x-2">
                    <Link to={`/admin/viewtraining/${training.id}`}>
                      <FontAwesomeIcon icon={faEye} className="text-blue-500" />
                    </Link>
                    <Link to={`/admin/edittraining/${training.id}`}>
                      <FontAwesomeIcon icon={faEdit} className="text-green-500" />
                    </Link>
                    <span onClick={() => handleDelete(training.id)} className="cursor-pointer">

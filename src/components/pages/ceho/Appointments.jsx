...
function AdminManageAppointments() {
  ...
  const fetchAppointments = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:8000/appointment/appointments/", axiosConfig);
  ...
  const filteredData = appointmentData.filter(
    appointment =>
      appointment.created_by.phone?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getRoleDisplayName = (role) => {
    switch (role) {
      ...
      case "ceho": return "Community Environment Officer";
      default: return role;
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex justify-between mb-4">
        {/* <Link to="/admin/createappointment" className="px-4 py-2 bg-blue-500 text-white rounded">
            ...
            {currentAppointments.length > 0 ? (
              currentAppointments.map(appointment => (
                <tr key={appointment.id} className="bg-white border-b">
                  ...
                  <td className="px-6 py-4">{getRoleDisplayName(appointment.address)}</td>
                  <td className="px-6 py-4">
                    {new Date(appointment.created_date).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4">
                  <Link to={`/admin/viewAppointmentDetails/${appointment.id}`}>
                      <FontAwesomeIcon icon={faEye} className="text-blue-500" />
                    </Link>
                    {/* <Link to={`/admin/editappointment/${appointment.id}`} className="px-4 py-2">

...
function Officer_Manage_Datasets() {
...
const handleDownload = {
  PDF: () => {
    ...
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Period: ${new Date().toLocaleDateString()} - Current`, 14, finalY);
    doc.text(`Showing: Page ${currentPage} of ${Math.ceil(filteredSortedData.length / datasetsPerPage)}`, 14, finalY + 7);
    doc.text(`Status: ${filteredSortedData.length} datasets found`, 14, finalY + 14);
    
    doc.save(`datasets_summary_${new Date().toISOString().split('T')[0]}.pdf`);
  },
  ...
  Excel: () => {
    ...
    // Save file
    XLSX.writeFile(workbook, `datasets_summary_${new Date().toISOString().split('T')[0]}.xlsx`);
  },
  ...
  CSV: () => {
    ...
    // Table data matching the displayed format
    const tableRows = currentDatasets.map(dataset => 
      `"${dataset.name}","${dataset.columns.length} columns","${dataset.size_human}","${new Date(dataset.last_modified * 1000).toLocaleDateString()}"`
    ).join("\n");
    ...
    const footerLines = [
      ...
      `Showing: Page ${currentPage} of ${Math.ceil(filteredSortedData.length / datasetsPerPage)}`,
    ...
    link.setAttribute("download", `datasets_summary_${new Date().toISOString().split('T')[0]}.csv`);
                  ...
                  <>
                    {filteredSortedData.length === 0 ? (
                      <div className="text-center py-12 bg-yellow-800 rounded-lg border border-yellow-700">
                        ...
                        <h3 className="text-xl font-medium text-yellow-300 mb-2">No datasets found</h3>
                        <p className="text-yellow-400">Try adjusting your search criteria</p>
                      </div>
                    ) : (
                        ...
                        <table className="w-full border-collapse">
                          <thead>
                            <tr className="bg-yellow-800 text-left">
                              <th className="p-4 font-semibold text-yellow-400 border-b border-yellow-700">Dataset Name</th>
                              <th className="p-4 font-semibold text-yellow-400 border-b border-yellow-700">Columns</th>
                              <th className="p-4 font-semibold text-yellow-400 border-b border-yellow-700">Size</th>
                              <th className="p-4 font-semibold text-yellow-400 border-b border-yellow-700">Last Modified</th>
                              <th className="p-4 font-semibold text-yellow-400 border-b border-yellow-700">Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                            {currentDatasets.map((dataset, index) => (
                              ...
                              >
                                <td className="p-4 font-mono text-yellow-200">{dataset.name}</td>
                                ...
                                <td className="p-4 text-yellow-300 font-mono">{dataset.size_human}</td>
                                <td className="p-4 text-yellow-300">
                ...
                {filteredSortedData.length > 0 && (
                  <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setCurrentPage(Math.min(Math.ceil(filteredSortedData.length / datasetsPerPage), currentPage + 1))}
                        disabled={currentPage === Math.ceil(filteredSortedData.length / datasetsPerPage)}
                        className={`px-3 py-1 rounded ${
                          currentPage === Math.ceil(filteredSortedData.length / datasetsPerPage)
                            ? "bg-yellow-700 text-yellow-500 cursor-not-allowed"
                            : "bg-yellow-800 text-yellow-300 hover:bg-yellow-700"
                        }`}
                      >
              ...
              <div className="bg-yellow-900 rounded-lg shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden border border-yellow-800">
                ...
                <div className="p-6 bg-yellow-800 border-t border-yellow-700">
                  <p className="text-yellow-300 text-sm">
                    Showing first {previewData.rows_shown || 0} rows of {previewData.total_rows || 0} total rows
                  </p>
                </div>
              </div>
            </div>
...
export default Officer_Manage_Datasets;

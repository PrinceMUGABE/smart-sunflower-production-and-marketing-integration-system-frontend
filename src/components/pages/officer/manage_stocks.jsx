...
function Officer_Manage_Stocks() {
  ...
  const fetchStocks = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://127.0.0.1:8000/stock/stocks/", {
...
const handleDownload = {
  PDF: () => {
    try {
      ...
      doc.text('Smart Sunflowe Production and marketing Integration System', 20, 25);
      
      doc.setFontSize(16);
      doc.setTextColor(60, 60, 60);
      doc.text('Sunflower Stock Management Report', 20, 35);
      ...
      if (filters.district) {
        doc.text(`• District: ${filters.district}`, 25, yPosition);
        yPosition += 5;
      }
      if (filters.quality_grade) {
        doc.text(`• Quality Grade: ${filters.quality_grade.replace("grade_", "Grade ").toUpperCase()}`, 25, yPosition);
        yPosition += 5;
      }
      if (searchQuery) {
      ...
      if (!filters.district && !filters.quality_grade && !searchQuery) {
      ...
      const summaryData = [
        ['Total Stock Records', filteredSortedData.length.toString()],
        ['Unique Districts', [...new Set(filteredSortedData.map(s => s.location?.district).filter(Boolean))].length.toString()],
        ['Total Current Quantity', `${filteredSortedData.reduce((sum, stock) => sum + parseFloat(stock.current_quantity || 0), 0).toFixed(2)} kg`],
        ['Total Original Quantity', `${filteredSortedData.reduce((sum, stock) => sum + parseFloat(stock.original_quantity || 0), 0).toFixed(2)} kg`],
        ['Overall Utilization Rate', `${filteredSortedData.length > 0 ? ((filteredSortedData.reduce((sum, stock) => sum + parseFloat(stock.current_quantity || 0), 0) / filteredSortedData.reduce((sum, stock) => sum + parseFloat(stock.original_quantity || 1), 0)) * 100).toFixed(2) : 0}%`]
      ];
      ...
      doc.setFontSize(12);
      doc.setTextColor(40, 40, 40);
      doc.text('Stock Details:', 20, finalY + 15);
      
      // Prepare table data
      const tableData = filteredSortedData.map((stock, index) => {
        const currentQty = parseFloat(stock.current_quantity || 0);
        const originalQty = parseFloat(stock.original_quantity || 0);
        const utilizationRate = originalQty > 0 ? ((currentQty / originalQty) * 100).toFixed(1) : '0.0';
        
        let status = 'Available';
        if (currentQty === 0) status = 'Empty';
        else if ((currentQty / originalQty) * 100 <= 20) status = 'Low Stock';
        
        return [
          (index + 1).toString(),
          new Date(stock.last_updated).toLocaleDateString(),
          `${stock.location?.district || 'N/A'}, ${stock.location?.sector || 'N/A'}`,
          `${stock.location?.cell || 'N/A'}, ${stock.location?.village || 'N/A'}`,
          currentQty.toFixed(2),
          originalQty.toFixed(2),
          `${utilizationRate}%`,
          stock.quality_grade?.replace("grade_", "Grade ").toUpperCase() || 'N/A',
          status
        ];
      });
      ...
      doc.autoTable({
        startY: finalY + 25,
        head: [['#', 'Last Updated', 'District, Sector', 'Cell, Village', 'Current (kg)', 'Original (kg)', 'Usage %', 'Grade', 'Status']],
        body: tableData,
        theme: 'striped',
        headStyles: { 
          fillColor: [234, 179, 8], 
        ...
        columnStyles: {
          0: { cellWidth: 12 },  // #
          1: { cellWidth: 22 },  // Last Updated
          2: { cellWidth: 25 },  // District, Sector
          3: { cellWidth: 25 },  // Cell, Village
          4: { cellWidth: 20 },  // Current
          5: { cellWidth: 20 },  // Original
          6: { cellWidth: 18 },  // Usage %
          7: { cellWidth: 18 },  // Grade
      ...
      // Quality Grade Distribution Summary (if there's space)
      const lastTableY = doc.lastAutoTable.finalY || finalY + 200;
      if (lastTableY < 250) { // If there's space on the page
        const gradeDistribution = filteredSortedData.reduce((acc, stock) => {
          const grade = stock.quality_grade?.replace("grade_", "Grade ").toUpperCase() || 'Unknown';
        ...
        if (Object.keys(gradeDistribution).length > 0) {
          doc.setFontSize(12);
          doc.setTextColor(40, 40, 40);
          doc.text('Quality Grade Distribution:', 20, lastTableY + 15);
          
          const gradeData = Object.entries(gradeDistribution).map(([grade, count]) => [
            grade,
            count.toString(),
            `${((count / filteredSortedData.length) * 100).toFixed(1)}%`
          ]);
          
          doc.autoTable({
            startY: lastTableY + 25,
            head: [['Quality Grade', 'Count', 'Percentage']],
            body: gradeData,
            theme: 'grid',
            headStyles: { fillColor: [234, 179, 8], textColor: [255, 255, 255] },
            alternateRowStyles: { fillColor: [252, 248, 227] },
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`© ${new Date().getFullYear()} Smart Sunflowe Production and marketing Integration System`, doc.internal.pageSize.width - 80, pageHeight - 10);
      }
      
      // Generate filename with timestamp
      const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
      const filename = `Stock_Management_Report_${timestamp}.pdf`;
      
      // Save the PDF
      doc.save(filename);
      
      showToast(`PDF report "${filename}" downloaded successfully!`, "success");
  ...
  Excel: () => {
    ...
    const excelData = filteredSortedData.map((stock, index) => {
      const currentQty = parseFloat(stock.current_quantity || 0);
      const originalQty = parseFloat(stock.original_quantity || 0);
      const utilizationRate = originalQty > 0 ? ((currentQty / originalQty) * 100).toFixed(1) : '0.0';
      ...
      return {
        ...
        'District': stock.location?.district || 'N/A',
        'Sector': stock.location?.sector || 'N/A',
        'Cell': stock.location?.cell || 'N/A',
        'Village': stock.location?.village || 'N/A',
        'Current Quantity (kg)': currentQty.toFixed(2),
        'Original Quantity (kg)': originalQty.toFixed(2),
        'Utilization Rate (%)': utilizationRate,
        'Quality Grade': stock.quality_grade?.replace("grade_", "Grade ").toUpperCase() || 'N/A',
        'Status': status,
        'Moisture Content (%)': stock.harvest?.moisture_content || 'N/A',
        'Oil Content (%)': stock.harvest?.oil_content || 'N/A'
      };
    });
    
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(excelData), "Stock_Data");
    
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    XLSX.writeFile(workbook, `Stock_Management_Report_${timestamp}.xlsx`);
    
    showToast("Excel report downloaded successfully!", "success");
    setDownloadMenuVisible(false);
  },
  CSV: () => {
    const csvData = filteredSortedData.map((stock, index) => {
      const currentQty = parseFloat(stock.current_quantity || 0);
      const originalQty = parseFloat(stock.original_quantity || 0);
      const utilizationRate = originalQty > 0 ? ((currentQty / originalQty) * 100).toFixed(1) : '0.0';
      ...
      return {
        'Serial_No': index + 1,
        'Last_Updated': new Date(stock.last_updated).toLocaleDateString(),
        'District': stock.location?.district || 'N/A',
        'Sector': stock.location?.sector || 'N/A',
        'Cell': stock.location?.cell || 'N/A',
        'Village': stock.location?.village || 'N/A',
        'Current_Quantity_kg': currentQty.toFixed(2),
        'Original_Quantity_kg': originalQty.toFixed(2),
        'Utilization_Rate_percent': utilizationRate,
        'Quality_Grade': stock.quality_grade?.replace("grade_", "Grade ").toUpperCase() || 'N/A',
        'Status': status
      };
    });
    ...
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    link.setAttribute("download", `Stock_Management_Report_${timestamp}.csv`);
                            ...
                            >
                              ...
                              <td className="px-6 py-4">
                                <div className="flex space-x-3">
                                  <Link
                                    to={`/officer/stocks/${stock.id}/movements`}
...
export default Officer_Manage_Stocks;
